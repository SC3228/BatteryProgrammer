...
class BQ28Z610(SmartBattery):
    NakError = OSError

    def __init__(self, device_name=None, log=None, interface=None, bd_serial=None):
        self.interface = interface   # is this the right thing to do?
        super(BQ28Z610, self).__init__(device_name, log, interface=self.interface)
    ...
    def mfg_blk_rd(self, alt_cmd):
        ...
        while retry_count:
            ...
            except (self.NakError, IOError, ValueError):
                if retry_count == 0:
                    raise
    ...
    def df_r(self, mem_addr, length):
            buffer = bytearray()
            while len(buffer) < length:
                data = self.df_rd(mem_addr)
                buffer += data
                mem_addr += len(data)
            # print(F'Read {len(buffer)} bytes, will return {len(buffer[:length])}.')
            return buffer[:length]

    def alt_mfg_cmd(self, alt_cmd, data=None):
        retval = self.interface.CY_SUCCESS
        retry_count = 5
        ...
        while retry_count:
            try:
                ...
                if self.interface.write_bytearray(self.BQ28Z610_DEVICE_ADDRESS, cmd_buff) != self.interface.CY_SUCCESS:
                ...
                if retval != self.interface.CY_SUCCESS:
    ...
    def gauge_en(self, enable):
        mfg_stat = self.manufacturing_status()
        if enable != mfg_stat['gauge']:
            self.alt_mfg_cmd(self.GAUGE_EN)
        mfg_stat = self.manufacturing_status()
        self.log.info('manufacturing_status().FET(%s)', mfg_stat['gauge'])
    ...
    ZEBRA_FULL_ACCESS_KEY = [0xFFFF, 0xFFFF]

    def unseal_device(self, key=(0x0414, 0x3672), to_mode=2):
        self.alt_mfg_cmd(key[0])
        self.alt_mfg_cmd(key[1])
        if (self.security_mode() != to_mode):
    ...
    def program_srec(self, srec):
        ...
        section1_size = 0x2000 # 8*1024
        section2_size = 0xE000 # 56*1024
        section3_size = 512
        ...
        for line in testlist_array:
            ...
            if not(line.startswith('S2') or line.startswith('S3')):
                ...
                data_length = data[0] - 5
                addr = int.from_bytes(data[1:5], 'big')

                for section in section_info:
                    if section['start'] <= addr < section['end']:
                        try:
                            struct.pack_into('{}s'.format(data_length), section['buffer'], addr - section['start'],
                                             data[5:])
                            found = True
                        except:
                            print(type(section['buffer']), addr - section['start'], type(data[5:]))
                            raise
                        break
