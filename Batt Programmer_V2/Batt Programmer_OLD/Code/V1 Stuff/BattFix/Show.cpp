...
void ShowPP(void)
{
	...
	sprintf(SNbuf,"%c%04d",letter+'A',SN);
	Serial.print(SNbuf);
...
void ShowPPP(void)
{
	...
	if (EX_CHARGE_TEMP_BLOCKS != (g_GiftedBattData.Block_Allocation_EEPROM & EX_CHARGE_TEMP_BLOCKS))  // Check if any of those blocks are marked as used
	{
		...
		Serial.print(g_GiftedBattData.ExCharge.DataType);
		Serial.print(F("   Stop temp: "));
		Serial.print(g_GiftedBattData.ExCharge.StopTemp);
		Serial.print(F("   Rec min temp: "));
		Serial.print(g_GiftedBattData.ExCharge.RecMinTemp);
		Serial.print(F("   Rec max temp: "));
		Serial.println(g_GiftedBattData.ExCharge.RecMaxTemp);
		...
		for (cnt = 0; cnt < 5; ++cnt)
		{
			...
			Serial.print(g_GiftedBattData.ExCharge.TempRange[cnt].StartTemp);
			...
			for (cnt1 = 0; cnt1 < 3; ++cnt1)
			{
				...
				Serial.print(cnt1);
				Serial.print(F(": "));
				Serial.print(g_GiftedBattData.ExCharge.TempRange[cnt].Voltage[cnt1]);
				Serial.print(F(" C"));
				Serial.print(cnt1);
				Serial.print(F(": "));
				Serial.print(g_GiftedBattData.ExCharge.TempRange[cnt].Current[cnt1]);
			}
...
void ShowGGregs(void)
{
	...
	Serial.print(g_GiftedBattData.AI);
	Serial.println(F("ma"));
	Serial.println();
