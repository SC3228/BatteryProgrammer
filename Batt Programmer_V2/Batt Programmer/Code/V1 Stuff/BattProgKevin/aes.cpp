...
static return_type aes_set_key( PGM_VOID_P key,
                         length_type keylen,
                         aes_context ctx[1] );
#endif
...
static return_type aes_encrypt( const unsigned char in[N_BLOCK],
...
static return_type aes_decrypt( const unsigned char in[N_BLOCK],
                         unsigned char out[N_BLOCK],
                         const aes_context ctx[1] );
#endif
...
#define gfm_e(x)   fe(x)
    ...
    static void copy_block_nn( void * d, PGM_VOID_P s, uint_8t nn )
    {
        uint_8t *dest = (uint_8t *)d;

        while( nn-- ) {
            *dest++ = pgm_read_byte(s++);
        }
    }
#endif
...
static return_type aes_set_key( PGM_VOID_P key, length_type keylen, aes_context ctx[1] )
{
    uint_8t cc, rc, hi;
    ...
    // Pre-clear ksch array.  This shouldn't be needed but it's the only way found to get
    // Coverity to shut-up about false uninitialized array accesses without having to
    // annotate the code every place aes is used.
    for (cc = 0; cc < (sizeof(ctx->ksch) / sizeof(ctx->ksch[0])); ++cc) {
        ctx->ksch[0] = 0;
    }
...
static return_type aes_encrypt( const unsigned char in[N_BLOCK], unsigned char  out[N_BLOCK], const aes_context ctx[1] )
{
    if( ctx->rnd )
    {
...
static return_type aes_decrypt( const unsigned char in[N_BLOCK], unsigned char out[N_BLOCK], const aes_context ctx[1] )
{
    if( ctx->rnd )
    {
        uint_8t s1[N_BLOCK], r;
...
void  MovI2Cdata(uint8_t *buffer1, uint8_t *buffer2, PGM_VOID_P buffer3, int dir)
{
	aes_context context;

	aes_set_key(buffer3, 16, &context);

	if (dir)
		aes_encrypt(buffer1, buffer2, &context);
	else
		aes_decrypt(buffer1, buffer2, &context);
}
