...
/*
Kevin	Added command to issue start with no stop and clk low first
		for testing EEPROM current draw
...
void loop()
{
	...
	if (Serial.available())
	{
		...
		if (done)
		{
			...
			{
#ifdef ZEBRA_MODE
			case SHUTDOWN_EEPROM:
				Serial.println(F("Doing proper EEPROM shutdown"));
				// Set I/O pins to inputs
				pinMode(SCL,INPUT);
				pinMode(SDA,INPUT);
				// Disable I2C to take over pin control
				Wire.end();
				// Drop SCL
				pinMode(SCL,OUTPUT);
				delay(1);
				...
				Serial.println(F("Done, remove battery, hit any key when done"));
				WaitForKey();
				pinMode(SDA,INPUT);
				delay(1);
				pinMode(SCL,INPUT);
				...
				break;
			case BREAK_EEPROM:
				Serial.println(F("Doing improper EEPROM shutdown"));
				// Set I/O pins to inputs
				pinMode(SCL,INPUT);
				pinMode(SDA,INPUT);
				// Disable I2C to take over pin control
...
//				Wire.end();
			...
			case AUTHENTICATE:  // Authenticate battery
				if (!BatteryType)
					...
					not_sup();
				else if (!NoCheckVoltage && BattVoltage() <= MIN_VOLTAGE) // And battery is <= 3V
					LowBatt();
				else
				{
					StartWire();
			...
			case READ_AUTH:  // Read auth chip into buffer
				...
				if (!BatteryType)
					NoType();
				else if (!NoCheckVoltage && BattVoltage() <= MIN_VOLTAGE) // Battery is <= 3V
					LowBatt();
				else
				{
					StartWire();
...
uint8_t CheckVoltage(void)
{
	...
	if ((!Batts[BatteryType].ActiveClk // If passive clk drive
				...
				&& BattVoltage() <= MIN_VOLTAGE) // And battery is <= 3V
	{
		LowBatt();
		return (1);
	}
...
void Status(void)
{
	if (BatteryType)
	{
		...
		if (Batts[BatteryType].EEPROM_Address)
		{
			...
			Serial.print(F(" Bytes of EEPROM storage"));
		}
		if (AUTH_CHIP_ADDR == Batts[BatteryType].EEPROM_Address)
...
void LowBatt(void)
{
	Serial.println(F("Battery voltage must be > 3.3V!"));
}
