...
static int GenPubKey(uint8_t ChipAddr, uint8_t slot, uint8_t *buf);
...
// Verify chip stuff
// ***************************************
const char ErrorAt[] PROGMEM = "Error at address: ";
#define ERROR_AT (const __FlashStringHelper*)ErrorAt
...
uint8_t AUTH_Read(uint8_t *buffer)
{
	uint8_t *data;
	int cnt, ret;
	uint8_t inch;
...
void AUTH_Write(uint8_t *buffer)
{
	int ret;
	int cnt;
	uint8_t inch;
...
uint8_t AUTH_ShowSN(void)
{
	...
	for (cnt = 0; cnt < 32; ++cnt)
		AuthSN[cnt] = CommBuf[cnt+1];

	// Get and check serial number
	SN = 0;
	sscanf((char *)(CommBuf+1),"SN: %d",&SN);
...
uint8_t CheckAuthChip(void)
{
	...
	{
		Serial.println();
		Serial.println(F("Validating Auth Chip"));
		Serial.print(F("Config area: "));
	}
...
*/
	...
	for (cnt = 0; cnt < KEY_LEN; ++cnt)
		DevicePubKey[cnt] = pgm_read_byte_far(g_PubKey+cnt);
	...
	if (Verbose)
		Serial.print(F("Device public key: "));
	...
	if (Verbose)
		Serial.print(F("Device private key: "));
...
static int GenPubKey(uint8_t ChipAddr, uint8_t slot, uint8_t *buf)
{
	...
	if (WriteAuthBytes(ChipAddr,CommBuf,10)) {
		Serial.println(F("Failed to send genkey command"));
		return(1);
	}
	...
	{
		Serial.println(F("Failed to get genkey response"));
		return(1);
	}
...
static int CheckCryptoConfig(void)
{
	...
	if (NoCheckSerialNum)
		if (AUTH_SN0 != CommBuf[0] || AUTH_SN1 != CommBuf[1] || AUTH_SN8 != CommBuf[12])
		{
			...
			Serial.print(F("Skiping SN check: "));
...
void AUTH_ShowAllData(void)
{
	...
	if (GenPubKey(Batts[BatteryType].CryptoAddress,DEVCIE_PRI,SlotBuf))
		return;

	Serial.print(F("Device Pub Key, from Prv Key"));
	for (cnt = 0; cnt < 64; ++cnt)
	{
		if (!(cnt % 16))
			Serial.println();
