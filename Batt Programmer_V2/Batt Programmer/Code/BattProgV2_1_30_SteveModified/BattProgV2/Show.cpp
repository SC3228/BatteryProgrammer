...
void Show27Z561regs(void)
{
	...
	// Show time since first use if we have it
	if (PPPV2_Valid)
	{
		val = NGGsecSinceMade - BD.NG.IT.Time[6] - BD.NG.IT.Time[5] - BD.NG.IT.Time[4] - BD.NG.IT.Time[3] - BD.NG.IT.Time[2] - BD.NG.IT.Time[1] - BD.NG.IT.Time[0];
		Serial.printf("Secs since first use: %d  In days: %d\r\n",val,val/86400);
	}
	else
		Serial.println("Time since first use not available, run VBD command.");
