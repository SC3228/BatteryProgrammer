...
/* Version history
1.2	Added SAMD vs SAME detection to allow moving NEO pixel pin

1.1	Fixed SN missmatch message from "does" to "doesn't".
	Set minimum serial numbers to over current allocations.
...
void setup()
{
	...
	{
	case 0: // SAMD51
		Serial.println("SAMD51 CPU");
		statusLED.setPin(NEO_PIN_D);
		break;
	case 3: // SAME51
		Serial.println("SAME51 CPU");
...
void EnterSerial(void)
{
	...
	{
		// Get serial number
		Serial.print("\r\nEnter serial number, must be 128-999 for Zebra internal units\r\n  and 1005-1099 for JDM units: ");
		if (-1 ==(sn = GetInt()))
			return;
		if ((sn < 128 || sn > 1099) || (sn >= 1000 && sn < 1005))
		{
		...
		else
		{
			...
			if (sn2 != sn)
			{
				Serial.printf("\r\nYou entered %d which doesn't match %d!\r\n",sn2,sn);
				sn = -1;
			}
			else
			{
