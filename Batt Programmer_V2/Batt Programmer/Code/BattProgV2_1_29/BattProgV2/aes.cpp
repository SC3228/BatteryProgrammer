...
#define gfm_e(x)   fe(x)
    ...
    static void copy_block( void *d, void *s )
    {
        #if defined( HAVE_UINT_32T )
    ...
    static void copy_block_nn(uint_8t *d, uint_8t* s, uint_8t nn)
    {
        while( nn-- ) {
            *d++ = *s++;
        }
...
static return_type aes_set_key(const uint8_t* key, length_type keylen, aes_context ctx[1] )
{
    ...
    block_copy_nn(ctx->ksch, (uint8_t *)key, keylen);
    hi = (keylen + 28) << 2;
    ctx->rnd = (hi >> 4) - 1;
...
void  MovI2Cdata(uint8_t *buffer1, uint8_t *buffer2, const uint8_t * buffer3, int dir)
{
	aes_context context;
